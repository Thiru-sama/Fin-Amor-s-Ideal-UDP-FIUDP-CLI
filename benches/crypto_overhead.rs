...
use fiudp_cli::internals::{derive_nonce, ChaChaEncryptor, Encryptor};
use fiudp_cli::types::{SessionId, ShardIndex};
use fiudp_cli::{HEADER_SIZE, NONCE_SIZE, SHARD_SIZE};
use rand::RngCore;
...
fn bench_encrypt_shard(c: &mut Criterion) {
    ...
    group.bench_function("chacha20_poly1305_encrypt_shard", |b| {
        b.iter(|| {
            ...
            let _tag = encryptor
                .encrypt_in_place(black_box(&nonce), black_box(&aad), black_box(&mut buffer))
                .unwrap();
        })
    });
